...
pub fn solve_bottom_row(unsolved_board: &mut Board, solved_board: &Board) {
    for x in 0..unsolved_board.get_width() {
        ...
        (pos_x, pos_y) = movement::move_row(unsolved_board, pos_x, pos_y, calculate_offset(pos_x, x));
...
pub fn solve_board_row(unsolved_board: &mut Board, solved_board: &Board, y: usize) {
    for x in 0..unsolved_board.get_width() {
        ...
        if last {
            ...
            movement::move_col(unsolved_board, unsolved_board.get_width() - 1, y, calculate_offset(y, pos_y));
        }
        ...
        // Elevator up
        (pos_x, pos_y) = movement::move_col(unsolved_board, pos_x, pos_y, calculate_offset(pos_y, y));
        ...
        if !last {
            ...
            // Elevator back down
            movement::move_col(unsolved_board, unsolved_board.get_width() - 1, 0, elevator_offset);
        }
    }
}
